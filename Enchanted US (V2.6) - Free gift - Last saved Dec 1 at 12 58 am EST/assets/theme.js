...
//                               '<img src="https://cdn.shopify.com/s/files/1/0267/0523/1943/t/89/assets/Remove-Delete_Icon.svg"/>'+
//                             '</a>'+
//                         '</div>'+
//             '</div>'+
//                       '</li>';
...
//   }
  ...
  var promotionPopup = {
    promotionPopup: function() {
      ...
      var chateauPopup = $('#chateau-popup');
      ...
      //  chateau popup
      if(chateauPopup.length){
        var chateauCookies = Cookies.get('chateauPopup');
        //         alert(chateauCookies);
        if(chateauCookies != undefined){
          $('#chateau-popup').css({"visibility":"hidden","opacity":"0"});
        }
        $('#chateau-popup button').on('click',function(){
          var passwordValue = $('#chateau-popup #CustomerPassword').val().toLowerCase();
          if(passwordValue=='ch21'){
          	$('#chateau-popup').css({"visibility":"hidden","opacity":"0"});
            Cookies.set('chateauPopup', 'open' ,{ expires: 1 });
          }else{
            $('#chateau-popup .input-group [data-form-error]').text('Please enter valid code.');
          }
        }); 
      }
    }
  ...
  var ajaxCartDrawer = {
    ...
    AjaxcartFreeProduct: function(isLoading){
      ...
      $('.cart-popup-item-wrapper .cart-popup-item').each(function(){
        ...
        if(itemVariant == '39538128945223'){
          hasFreeProduct = true;
        }
      });
      if(cartTotal>49899 && !hasFreeProduct){
        var freeProduct = {
          "id": 39538128945223,
          "quantity": 1
        }
        $.ajax({
          ...
          success: function () {
            $.get('/cart?view=mm-ajax', function(cart) {
              ...
              var NewCartCount = parseInt($('.cart-title h5 span').text());
      ...
      } else if(hasFreeProduct && cartTotal<49900 ){
        var freeProduct = {
          "id": 39538128945223,
          "quantity": 0
        }
    ...
    AjaxcartHideLoader: function(isLoading){
      if(!isLoading){
        ...
        $('.product_button button[name="add"],.mm-product-card .add-to-cart-button').find("[data-add-to-cart-text]").removeClass("hide");
        $('.product_button button[name="add"],.mm-product-card .add-to-cart-button').find("[data-loader]").addClass("hide");
      }
    },
    ...
    AjaxcartQuantityHandler: function() {
      ...
      $(document).on('click', '.mm-ajaxcart-remove-item', function() {
        var RemoveLine = parseInt($(this).attr('data-line')),
            params = {quantity: 0,line: RemoveLine};
        ajaxCartDrawer.AjaxcartChangeHandler(params);
      })
    }
  }

  $(document).on('click', '.mm-ajaxcart-open,.header__right .cart,.site-header__cart,.cart [data-cart-count]', function(e) {
    e.preventDefault();
    ajaxCartDrawer.AjaxcartRender();
  });
  ...
  $(document).on('click', '.product_button button[name="add"],.mm-product-card .add-to-cart-button', function(e) {
    e.preventDefault();
    ...
    function ajaxAdd(data) {
      $.ajax({
        ...
        data: data,
        beforeSend: function () {
          $this.find("[data-add-to-cart-text]").addClass("hide");
          $this.find("[data-loader]").removeClass("hide");
        },
        success: function () {
          ajaxCartDrawer.AjaxcartRender();
          ...
          $.getJSON('/cart.js', function(cart) {
            ...
            $('.cart-popup__heading span').text(currentCartCount);
          });
        },
        error: function (error) {
...
$(document).ready(function (){
  ...
  if(webpageLocation.indexOf('/pages/the-chateau')>-1){
