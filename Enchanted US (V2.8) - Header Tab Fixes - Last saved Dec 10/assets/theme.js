...
// 	alert("test");
  ...
  var template ='<li class="item" id="cart-item-{ID}">'+
                      ...
                      '<div class="pro-name-and-price">'+
                        ...
                        '<div class="price-qty-remove">'+
                          ...
                          '{PRICE}'+
                          '</span>'+
              '<div class="option quantity-box size-box size{VARIANT} text-center">'+
              '<span class="qty">Size: <span class="qty-number">{VARIANT}</span></span>'+
                          '</div>'+
                          '<div class="option quantity-box text-center">'+
                            '<span class="qty">Qty: <span class="qty-number">{QUANTITY}</span></span>'+
                          '</div>'+
              
               '<a href="javascript:void(0)" title="remove" class="btn-remove">'+
                              '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" id="Capa_1" x="0px" y="0px" width="18" height="24" viewBox="0 0 490.646 490.646" style="enable-background:new 0 0 490.646 490.646;" xml:space="preserve"><g><g><style>.btn-remove path {fill: #12204A;}.btn-remove path.bottm {stroke: #ffffff;stroke-width: 18px;}</style><path class="lid" d="M399.179,67.285l-74.794,0.033L324.356,0L166.214,0.066l0.029,67.318l-74.802,0.033l0.025,62.914h307.739L399.179,67.285z     M198.28,32.11l94.03-0.041l0.017,35.262l-94.03,0.041L198.28,32.11z"></path><path class="bottm" d="M91.465,490.646h307.739V146.359H91.465V490.646z M317.461,193.372h16.028v250.259h-16.028V193.372L317.461,193.372z     M237.321,193.372h16.028v250.259h-16.028V193.372L237.321,193.372z M157.18,193.372h16.028v250.259H157.18V193.372z"></path></g></g><g></g><g></g><g></g><g></g><g></g><g></g><g></g><g></g><g></g><g></g><g></g><g></g><g></g><g></g><g></g></svg>'+
                            '</a>'+
                        '</div>'+
            '</div>'+
                      '</li>';
  var miniProductList = $("#dropdown-cart .mini-products-list");
  ...
  if (cart.item_count > 0) {
    for (var i = 0; i < cart.items.length; i++) {
      ...
      item = item.replace(/\{IMAGE\}/g, cart.items[i].image);
      item = item.replace(/\{PRICE\}/g, theme.Currency.formatMoney((cart.items[i].final_price)*cart.items[i].quantity, theme.moneyFormat));
      
      miniProductList.append(item);
    }
  }
  //   console.log(cart);
  $(".summary .total .price").html(theme.Currency.formatMoney(cart.total_price, theme.moneyFormat));
  $(".ajax-loader").hide();
}

$(document).ready(function(){

  $(".close-cart,.wrapper-overlay,.continue-shopping").on("click",function(){
    $("html").removeClass("cart-show");
  });
  $(".shopify-payment-button__button").on("click",function(){
  	$(".shopify-payment-button__button .checkout-text").hide();
    $(this).find("[data-loader]").removeClass('hide');
    $(".shopify-payment-button__button span:last-child").removeClass("hide");
  })
  $(".btn-checkout").on("click",function(){
  	$(".btn-checkout .checkout-text").hide();
    $(this).find("[data-loader]").removeClass('hide');
    $(".btn-checkout span:last-child").removeClass("hide");
  })
  
  $(".header__container .cart").on("click",function(e){
    var checkCartPage = $('body').data('template');
    if(checkCartPage.indexOf('cart')>-1){
  ...
  $("html,body").on("click",".btn-remove",function(e){
    e.preventDefault();
    e.stopPropagation();
      ...
      success: function() {
        $.getJSON('/cart.js').then(
          function(cart) {
            ...
            if(cart.item_count > 0){
              $(".has-items,.actions").show();
              $(".no-items").hide();
            }else{
              $(".has-items,.actions").hide();
              $(".no-items").show();
            }
            $('[data-cart-count]').html(cart_count);
          }
        );
      },
  ...
  $("html,body").on("click",".mm-product-card .add-to-cart-button",function(e){
    e.preventDefault();
    e.stopPropagation();
    var variantId = $(this).closest('form').attr('data-id');
    var $_this = $(this);
    $_this.find('.add-to-cart-text').addClass('hide');
    $_this.find('[data-loader]').removeClass('hide');
    var params = {          
      quantity: 1,
      ...
      success: function() {
        $.getJSON('/cart.js').then(
          function(cart) {
            ...
            $('[data-cart-count]').html(cart_count);
            $('.header__container .cart').trigger('click');
            $_this.find('.add-to-cart-text').removeClass('hide');
            $_this.find('[data-loader]').addClass('hide');
          }
        );
      },
      error: function(error) {
  ...
  setTimeout(function(){
    $(document).on('click','.evm-ws__cart-submit',function(){
      $(this).html('<span data-loader=""><svg aria-hidden="true" focusable="false" role="presentation" class="icon icon-spinner" viewBox="0 0 20 20"><path d="M7.229 1.173a9.25 9.25 0 1 0 11.655 11.412 1.25 1.25 0 1 0-2.4-.698 6.75 6.75 0 1 1-8.506-8.329 1.25 1.25 0 1 0-.75-2.385z" fill="#919EAB"></path></svg></span>');
      var curevl=$(this); 
      setTimeout(function(){
        $.getJSON('/cart.js').then(
          function(cart) {
        ...
        curevl.html('<span>ADD TO BAG</span>');
        $("html").addClass("cart-show");
      }, 1000);  
    });
  }, 2000);
...
slate.Variants = (function() {
  ...
  function Variants(options) {
    ...
    if(!this.currentVariant.available){
      $(".product_button-video").hide();
      $('.est_delivery_date_pdp').addClass('hide');
      $('#BIS_trigger.BIS_trigger').show();
    }
...
theme.Cart = (function() {
  ...
  Cart.prototype = _.assignIn({}, Cart.prototype, {
    ...
    _setLineItemImage: function($item, featuredImage) {
      ...
      if (sizedImageUrl) {
        $image.attr('data-image', 'text').attr('alt', featuredImage.alt).attr('src', sizedImageUrl).removeClass(classes.hide);
      } else {
        $image.remove();
      }
    },
  ...
  Product.prototype = _.assignIn({}, Product.prototype, {
    ...
    _addItemToCart: function(data) {
      var _showSuccess = function() {
      	let messageWrapper = $('[data-cart-success-message]');
        if(messageWrapper.length) {
          messageWrapper.addClass('has--active');
        }
        
        let closeButton = $('[data-cart-success-close]');
        if(closeButton.length) {
          closeButton.on('click', function() {
            messageWrapper.removeClass('has--active');
          })         
        }
        setTimeout(function() {
          messageWrapper.removeClass('has--active');
...
//             _toggleBodyCart();
    ...
    _getVariantOptionList: function(variantOptions) {
      ...
      // console.log(variantOptionListHTML);
    ...
    _liveRegionText: function(variant) {
      ...
      if (variant.compare_at_price > variant.price) {
		...
		//  Percent off js start
        var discountOff = 100 - ((variant.price / variant.compare_at_price) * 100);
        discountOff = Math.round(discountOff);           
        $('.single__product--description .price-item--discount').html('('+discountOff+'% OFF)');
        //  Percent off js end
      }
    ...
    _updateAddToCart: function(evt) {
      ...
      if (variant) {
        ...
        var minStock = $(selectors.variantInventory).attr('data-min-qty');
        
        if (inventory_level == 0){
          if (variant.available) {
            $('.variant-inventory').css('color', '#b5323d').html('Out of Stock!').hide();  
          }else{
            $('.variant-inventory').css('color', '#b5323d').html('Out of Stock!').show();  
          }
        } else if (inventory_level > minStock){
          $('.variant-inventory').css('color', '#379f6a').html('In Stock!').show();
        } else {
          $('.variant-inventory').css('color', '#b5323d').html('Only ' + inventory_level + ' left in Stock!').show();
        }
        
        if (variant.available) {
          this.$addToCart.attr({
            'aria-label': theme.strings.addToCart,
            'type': 'submit',
            'data-add-to-cart': ''
          });
          ...
          //Remove class for soldout popup
          this.$addToCart.removeClass('soldout-popup');
          this.$addToCart.off("click");
          $('.product_button-cart .add-to-cart-button').show();
          $('.vault-sold-out').hide();
          $('.est_delivery_date_pdp').removeClass('hide');
          $('#BIS_trigger.BIS_trigger').hide();
        } else {
          $('.product_button-cart .add-to-cart-button').hide();
          $('.vault-sold-out').show();
          $('.est_delivery_date_pdp').addClass('hide');
          $('#BIS_trigger.BIS_trigger').show();
          ...
          this.$addToCart.attr({
            'aria-label': theme.strings.soldOut,
            'type': 'button'
          });
          
          this.$addToCart.removeAttr('data-add-to-cart');
          ...
          this.$addToCart.addClass('soldout-popup').attr('data-size', variant.title);
          $('.soldout-popup').on('click', function(e) {
            e.preventDefault();
            
            var size = $(this).attr('data-size');
            
            $('#item-size').text(size);
            $('#item-size').parent('label').next('input').val(size);
    ...
    _updateSKU: function(evt) {
      ...
      $(".product_button-video .btn").attr('data-sku',variant.sku);
    },
...
$(document).ready(function() {
  ...
  //  Filter remove code in desktop and mobile 
  if($(window).width()>767){
    $('.mm-filter-sidebar').remove();
  }else{
  ...
  var currentPageTemplate = $('body').data('template');
  // console.log(currentPageTemplate);
  
  if(currentPageTemplate != 'collection'){
    //Wishlist
  ...
  // Last minute deal banner js start
  window.setInterval(function() {
    $(".custom-full-width-banner .animated_txt").each(function() {
        $(this).toggleClass('red_color')
    });
    $(".innerpage__banner .animated_txt").each(function() {
      $(this).toggleClass('red_color')
    });
  }, 800);
  // Last minute deal banner js end
});
...
$(document).ready(function() {
  ...
  var ProductPage = {
    ...
    initAccordion: function() {
      $(".accordion-header:first-of-type").addClass("active").next().show();

      $(".product_description--accordion").on("click", ".accordion-header", function() {
        $(this).toggleClass("active").next().slideToggle();
        $(".accordion-header").not(this).removeClass("active").next().slideUp();
      });
    },
    initMagnificPopup: function() {
      var PopupButton = $('.js-content-popup');      
    ...
    readMoreDescription: function() {
      $('.product_desc .read-more-desc').on('click', function(e){
        e.preventDefault();
        var readText = $(this).text();
        $('.product_desc .product_desc-short').toggleClass('hide');
        $('.product_desc .product_desc-full').toggleClass('show');
        if(readText.indexOf('Read more')>-1){
          $(this).text('Read less...');
        }
        else {
          $(this).text('Read more...');
  ...
  var AjaxRemoveFromCart = {
    init: function() {
      $(document).on('click', '.js-remove-from-cart', function(){ 
        var $self = $(this);
        var lineItem = $(this).attr('data-line-item');
        var _showSuccess = function() {
          $self.parents('.cart__content').hide();
          let messageWrapper = $('[data-cart-delete-message]');
          if(messageWrapper.length) {
            messageWrapper.addClass('has--active');
          }
          let closeButton = $('[data-cart-success-close]');
        ...
        var _updateCount = function() {
          $.getJSON('/cart.js').then(
            function(cart) {
              let cart_count = cart.item_count; 
              $('[data-cart-count]').html(cart_count);
              $('[data-cart-subtotal]').html( theme.Currency.formatMoney(cart.total_price,theme.moneyFormat));
            }
          );
        }
        var params = {          
  ...
  var HomePage = {
    initSliders: function() {
      var VaultSlider = $('.introducing__slider');
      var CategorySlider = $('.shopby__category--slider');
      var ClinteleSlider = $('.clientele__section--slider');
      var PrincessStylesSlider = $('.theme__princess--styles');
      var CareerSlider = $('.career__slider--inner');
      
      if(VaultSlider.length) {
        VaultSlider.slick({
          dots: true,
          arrows: false,
          infinite: true,
      ...
      if(ClinteleSlider.length) {
        ClinteleSlider.slick({
          dots: false,
          arrows: true,
          infinite: true,
          speed: 300,
      ...
      if(PrincessStylesSlider.length) {
        PrincessStylesSlider.slick({
          dots: true,
          arrows: false,
          infinite: true,
          speed: 300,
    ...
    initPopup: function() {
      $('.video_placeholder a').magnificPopup({
        type: 'iframe',
      })
    },
    initBannerSlider: function() {
      ...
      $('.thumbnail__slider').slick({
        ...
        arrows: true,
        prevArrow:'<button class="slick-prev slick-arrow" aria-label="Previous" type="button" ><svg version="1.1" id="Capa_1" width="18" height="18" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 256 256" style="enable-background:new 0 0 256 256;" xml:space="preserve"><g><g><polygon points="207.093,30.187 176.907,0 48.907,128 176.907,256 207.093,225.813 109.28,128 "/></g></g><g></g><g></g><g></g><g></g><g></g><g></g><g></g><g></g><g></g><g></g><g></g><g></g><g></g><g></g><g></g></svg></button>',
        nextArrow:'<button class="slick-next slick-arrow" aria-label="Next" type="button" ><svg version="1.1" id="Capa_1" width="18" height="18" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" width="306px" height="306px" viewBox="0 0 306 306" style="enable-background:new 0 0 306 306;" xml:space="preserve"><g><g id="chevron-right"><polygon points="94.35,0 58.65,35.7 175.95,153 58.65,270.3 94.35,306 247.35,153 "/></g></g><g></g><g></g><g></g><g></g><g></g><g></g><g></g><g></g><g></g><g></g><g></g><g></g><g></g><g></g><g></g></svg></button>',
        autoplay: true,
        dots: false,
        pauseOnHover: true,
        pauseOnFocus: true,
  ...
  var needSoon = {
    initNeedSoon: function() {
      $('.need_soon').on('click', function() {
      	$(this).next('label').toggle();
      })
    }
  }
  ...
  var notifyMe = {
    initNotifyMe: function() {
      $('.soldout-popup').on('click', function() {
        var np = $('#notify-popup');
        if (np.length) {
          $.magnificPopup.open({
            items: {
  ...
  var promotionPopup = {
    promotionPopup: function() {
      ...
      var pageUrlLocation = window.location.href;
      if(theme.isHasCustomer || pageUrlLocation.indexOf('account')>-1){
      }
      else{
        if(promotionPopup.length){
          $('.product__price .early-access-text .sign-in-popup').on('click',function(e){
            e.preventDefault();
            $('#promotion-popup').css({"visibility":"visible","opacity":"1"});
          });
          if(pageUrlLocation.indexOf('black-friday-early-access-sale')>-1){
            $('#promotion-popup').css({"visibility":"visible","opacity":"1"});
          }

          if(pageUrlLocation.indexOf('black-friday-early-access-sale')<0){
            $("#promotion-popup").click(function(e){
              if($(e.target).closest(".popup-content").length<1){
            ...
            $("#promotion-popup .popup-content a").click(function(e){
              $(this).closest('.overlay').css({"visibility":"hidden","opacity":"0"});
              Cookies.set('popup', 'open' ,{ expires: 1 });
            });
            $("#promotion-popup .close-the-popup").click(function(e){
              e.preventDefault();
              $(this).closest('.overlay').css({"visibility":"hidden","opacity":"0"});
              Cookies.set('popup', 'open' ,{ expires: 1 });
            });
  ...
  var rewardPage = {
    initAccordion: function() {
      $(".rb-faq-wrapper .accordion-header:first-of-type").addClass("active").next().show();

      $(".rb-faq-wrapper").on("click", ".accordion-header", function() {
        $(this).toggleClass("active").next().slideToggle();
        $(".accordion-header").not(this).removeClass("active").next().slideUp();
      })
    }
  ...
  AccountMenu.close();
  AjaxAddToCart.init();
  AjaxRemoveFromCart.init();
  CartPage.initQuantityHandler();
  HomePage.initPopup();
  HomePage.initSliders();
  HomePage.initBannerSlider();
  ProductPage.initSlider(); 
  ProductPage.initAccordion();
  ProductPage.initMagnificPopup();
  ProductPage.readMoreDescription();
  needSoon.initNeedSoon();
  notifyMe.initNotifyMe();
  promotionPopup.promotionPopup();
  rewardPage.initAccordion();


  // Second Ready Function Copied below
  if(document.referrer.indexOf(window.location.hostname) != -1){
    var referrer =  document.referrer;
    // console.log(document.referrer);
    if(referrer.indexOf('/account')>-1){
//       $("#redirect_url").val('');
    }else{
  ...
  // FOR REVIEWS BY MARMETO
  $('.yotpo-main-widget').hide();
  $(document).on('click', '.yotpo.bottomLine', function(e) {
    $('.yotpo-main-widget').slideToggle();
    e.preventDefault();
  });
  
  $('.product-grid-slider .grid--view-items').slick({
    dots: false,
    infinite: true,
    speed: 500,
    slidesToShow: 3,
    ...
    arrows: true,
    rows: 0,
    prevArrow:'<button class="slick-prev slick-arrow" aria-label="Previous" type="button" ><svg xmlns="http://www.w3.org/2000/svg" width="13" height="24.073" viewBox="0 0 13 24.073"><path id="Right_arrow" data-name="Right arrow" d="M11.264,23a.744.744,0,0,0,.52-.211.7.7,0,0,0,0-1.016L1.615,11.839a.474.474,0,0,1,0-.678L11.784,1.227a.7.7,0,0,0,0-1.016.746.746,0,0,0-1.04,0L.575,10.145a1.883,1.883,0,0,0,0,2.71l10.169,9.934a.744.744,0,0,0,.52.211" transform="translate(0.5 0.573)" fill="#bc913f" stroke="#bc913f" stroke-width="1"/></svg></button>',
    nextArrow:'<button class="slick-next slick-arrow" aria-label="Next" type="button" ><svg xmlns="http://www.w3.org/2000/svg" width="13" height="24.073" viewBox="0 0 13 24.073"><path id="Right_arrow" data-name="Right arrow" d="M.736,23a.744.744,0,0,1-.52-.211.7.7,0,0,1,0-1.016l10.169-9.934a.474.474,0,0,0,0-.678L.216,1.227a.7.7,0,0,1,0-1.016.746.746,0,0,1,1.04,0l10.169,9.934a1.883,1.883,0,0,1,0,2.71L1.256,22.789A.744.744,0,0,1,.736,23" transform="translate(0.5 0.573)" fill="#bc913f" stroke="#bc913f" stroke-width="1"/></svg></button>',
    responsive: [
      {
        breakpoint: 600,
        settings: {
  ...
  $('.banner-product-grid-slider .grid--view-items').slick({
    dots: false,
    infinite: true,
    speed: 500,
    slidesToShow: 3,
    ...
    arrows: true,
    rows: 0,
    prevArrow:'<button class="slick-prev slick-arrow" aria-label="Previous" type="button" ><svg version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 256 256" style="enable-background:new 0 0 256 256;" xml:space="preserve"><g><g><polygon points="207.093,30.187 176.907,0 48.907,128 176.907,256 207.093,225.813 109.28,128 "/></g></g><g></g><g></g><g></g><g></g><g></g><g></g><g></g><g></g><g></g><g></g><g></g><g></g><g></g><g></g><g></g></svg></button>',
    nextArrow:'<button class="slick-next slick-arrow" aria-label="Next" type="button" ><svg version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" width="306px" height="306px" viewBox="0 0 306 306" style="enable-background:new 0 0 306 306;" xml:space="preserve"><g><g id="chevron-right"><polygon points="94.35,0 58.65,35.7 175.95,153 58.65,270.3 94.35,306 247.35,153 "/></g></g><g></g><g></g><g></g><g></g><g></g><g></g><g></g><g></g><g></g><g></g><g></g><g></g><g></g><g></g><g></g></svg></button>',
    responsive: [
      {
        breakpoint: 1023,
        settings: {
  ...
  $('.custom-content-wrapper .custom-content').slick({
    dots: false,
    infinite: true,
    speed: 500,
    slidesToShow: 1,
  ...
  $('.collectin-description a,.single__product--description .bundle-link').click(function(e){
    e.preventDefault();
    $('html, body').animate({
  ...
  $('#shopify-section-mothers-feature-row .scroll-down').click(function(e){
    e.preventDefault();
    $('html, body').animate({
      scrollTop: $('#shopify-section-young-mom-banner').offset().top - 120
    }, 500);
    return false;
  });
  function showShippingMsg(Size, Price){
    if(Size == 5 || Size == 9){
      $(".ring_size-selection label .infor").css("display","inline-block");
      $(".est_delivery_date_pdp").hide();
    }else{
      $(".ring_size-selection label .infor").hide();
      $(".est_delivery_date_pdp").css("display","block");
    }
  }
  var variantSize = $(".product-form__input").find("option:selected").val();
  var variantPrice = $('[name="id"]').find("option:selected").data('price');
  $(".product-form__input").on("change",function(){
    var onChagneSize = $(this).find("option:selected").val();
    var productVariantPrice = $('[name="id"]').find('option:selected').data('price');
    showShippingMsg(onChagneSize,productVariantPrice);
  });
  showShippingMsg(variantSize,variantPrice);
  
  
  $(".product_button-video .btn").on("click",function(){
    var currentSKU = $(this).attr("data-sku");
    localStorage.setItem("currentSKU", currentSKU);
    window.location.href = "/pages/video-consultation";
  });
  $(".header__right .page_menu a,.header__right a.medium-up--hide").on("click",function(){
  	localStorage.removeItem("currentSKU");
  });

  $('.close-promotion,.promotion_container img,.mm-popup--opened:before').on('click', function(e) {
    $('body').removeClass('mm-popup--opened');
  });

  $('body').on('click', function(e) {
    if($("body").hasClass("mm-popup--opened")){
      $('body').removeClass('mm-popup--opened');
    }
  });
  
  $("#the-vault2").click(function(e){
    if($(e.target).closest(".popup").length<1){
      $(this).css({"visibility":"hidden","opacity":"0","display":"none"});
    }
  });

  $("#the-vault2 .close").click(function(e){
    $("#the-vault2").css({"visibility":"hidden","opacity":"0","display":"none"});
  });

  $(".monthly-payment a").click(function(e){
    e.preventDefault();
    $("#the-vault2").css({"visibility":"visible","opacity":"1","display":"block"});
  });
  
  // zip code pop up variables
  // var countryRedirect = Cookies.get('countryRedirect');
  // var countryCode;
  var myVar2 = setInterval(myTimer2, 1000);
  var count;
  
  var popup = Cookies.get('popup');
  var subPopup = Cookies.get('subscribed-popup');
  
  
  var getCookiesTimer = Cookies.get('getCookiesTimer');
  if(getCookiesTimer == null || getCookiesTimer == undefined){
    count = 0;
  }else{
    count = parseInt(getCookiesTimer);
  }
  
  function myTimer2() {
    count += 1;
    // var setCookiesTimer = Cookies.set('getCookiesTimer',count);
	console.log(count);
    var pageUrlLocation = window.location.href;
    if(count == 5){
      // var setCookiesTimer = Cookies.set('getCookiesTimer',0);
      clearInterval(myVar2);
      if(popup != 'open' && subPopup != 'reopen' && pageUrlLocation.indexOf('account')<0){
        $("#promotion-popup").css({"visibility":"visible","opacity":"1"});
      }      
    }
    /*if(count>=15){
      ...
      $.ajax({
        dataType: "json",
        url: "https://pro.ip-api.com/json/?fields&key=P4BolzQRyPxs0qh",
        success: function(data){
          var countryCode = data.countryCode; 
          var zip = data.zip;
          console.log(countryCode,zip);
          if(zip == 'NW1' || countryCode == 'BR'){
            return false;
          }
          if(countryCode != "US" && countryCode != "IN"){
            // console.log(countryCode);
            $("#the-vault3").css({"visibility":"visible","opacity":"1"});
            return false;
          }
        }
      });
  ...
  // Get the video
  var video = document.getElementById("myVideo_desktop");
  ...
  var videoHeader = document.getElementById("myVideo_web-header");
  ...
  // Villain Page content js start
  $('.banner_explore').on('click', function(){
    $(this).closest('.banner-sketch-wrapper').find('.sketch_images').slideToggle();
  });
  // Villain Page content js end

  $('.mm-badges .valut-badge').on('touchstart', function() {
	$('.mm-badges .valut-badge').removeClass('hover');
    $('.mm-badges .valut-badge').addClass('hover');
  }); 
  
  // Register form validation
  function ValidateRegisterForm() {
    var validation = true;
    var firstName = $('[name="customer[first_name]"]'),
        title = $('#title-dropdown'),
        lastName = $('[name="customer[last_name]"]'),
        dob = $('[name="customer[note][birthday]"]'),
        email = $('[name="customer[email]"]'),
        password = $('[name="customer[password]"]'),
        confirmPassword = $('#RegisterForm-confirm-password');
    
    
    
    $('#register___page input').removeClass('input--error');
    $('#register___page [data-form-error]').html('').hide();
    title.closest('.input-group').removeClass('input--error');
    
    

    var emailPattern = /^[a-zA-Z0-9\-_]+(\.[a-zA-Z0-9\-_]+)*@[a-z0-9]+(\-[a-z0-9]+)*(\.[a-z0-9]+(\-[a-z0-9]+)*)*\.[a-z]{2,4}$/;
//     alert(title.val());
    var titelValue = title.val();
    if(titelValue.indexOf('Select title')>-1) {
      title.closest('.input-group')
      .addClass('input--error')
      .find('[data-form-error]')
      .html('Please select title')
      .show();
      validation = false;
    }
    ...
    if(dob.val() == '') {
      dob
      .addClass('input--error')
      .parent().next('[data-form-error]')
      .html('Please enter your birth date')
      .show();
      validation = false;
    }
    ...
    if(password.val() == '') {
      ...
      .addClass('input--error')
      .next('[data-form-error]')
      .html('Password must have more than 6 characters')
      .show();
      validation = false;
    } else{
      password
    ...
    if(confirmPassword.val() == '') {
      ...
      .addClass('input--error')
      .next('[data-form-error]')
      .html('Confirm password should match with password')
      .show();
      validation = false;
    }
  ...
  $('#register___page .btn-border').on('click', function(e) {
  ...
  function ValidatePopupLoginForm() {
    var validation = true;
    var email = $('#promotion-popup #customer_login [type="email"]'),
        password = $('#promotion-popup #customer_login [type="password"]');

    var emailPattern = /^[a-zA-Z0-9\-_]+(\.[a-zA-Z0-9\-_]+)*@[a-z0-9]+(\-[a-z0-9]+)*(\.[a-z0-9]+(\-[a-z0-9]+)*)*\.[a-z]{2,4}$/;
    if(!emailPattern.test(email.val())) {
      email.addClass('input--error').next('[data-form-error]').html('Please enter a valid email address').show();
      validation = false;
    }
    if(password.val().length < 1) {
      password.addClass('input--error').next('[data-form-error]').html('Please enter a valid password').show();
      validation = false;
    }
    return validation;
  }
  
  $('#promotion-popup #customer_login .login-btn').on('click', function(e) {
    if(ValidatePopupLoginForm() == false){
      e.preventDefault();
      console.log(123);
    }
    return ValidatePopupLoginForm();
  });
  ...
  // Header sticky on scroll up and hide on scroll down -- code
  var didScroll;
  var lastScrollTop = 0;
  var delta = 5;
  var navbarHeight = $('#shopify-section-custom-header').outerHeight();
  ...
  function hasScrolled() {
    ...
    if (st > lastScrollTop && st > navbarHeight){
      // Scroll Down
      $('#shopify-section-custom-header').removeClass('nav-down').addClass('nav-up');
    } else {
      // Scroll Up
      if(st + $(window).height() < $(document).height()) {
        $('#shopify-section-custom-header').removeClass('nav-up').addClass('nav-down');
      }
    }
  ...
  $(document).on('click','.desktop-filter .filter-option-heading', function(){
    $(this).parents('.mm-filter-nav').siblings().find('.filter-content').slideUp(300);
    $(this).next('.filter-content').slideToggle(300);
  });
  $("body").click(function(e){
    if($(window).width()>767){
      if($(e.target).closest(".desktop-filter").length<1){
  ...
  // Third Ready Function pasted below
  // Homepage
  var webpageLocation = window.location.href;
  var homePage = {
    ...
    initJewelryInfo: function() {
      $('.jewelry-info-section .btn-show-info').on('click', function (e){
        e.preventDefault();
        $(this).parent('.content').siblings('.jewelry-info').slideDown();
        $('.content .text').hide();
        $(this).addClass('hide');
        $('.jewelry-info-section .btn-hide-info').removeClass('hide');
      });
      $('.jewelry-info-section .btn-hide-info').on('click', function (e){
        e.preventDefault();
        $(this).parent('.jewelry-info').slideUp();
        $(this).addClass('hide');
        $('.jewelry-info-section .btn-show-info').removeClass('hide');
        $('.content .text').show();
      });
      
    },
    
    customFullWidthSlider: function() {
      var fullWidthBannerSlider = $('.full-width-banner-slider');
      if(fullWidthBannerSlider.length) {
        fullWidthBannerSlider.slick({
          dots: true,
          arrows: false,
          infinite: true,
          speed: 500,
    ...
    initShopTheLookSlider: function() {
      var shopTheLookSlider = $('.shop-the-look-slider');
      if(shopTheLookSlider.length) {
        shopTheLookSlider.slick({
          dots: false,
          arrows: true,
          infinite: false,
          speed: 500,
    ...
    shopTheLook: function() {
      var $this = $('.shop-all-products');
      var lookHtml = '';
      $('.shop-the-look__settings > div').appendTo('#shop-the-look__wrapper');
      var lookDrawer;
      $(document).on('click','.shop-all-products',function(){
        lookDrawer = $(this).attr('data-shop-look-drawer');
        $('html').addClass('lookbook-show');
        $('body').find('#shop-the-look__wrapper #'+lookDrawer).addClass("show-looks");
      });
      $('.wrapper-overlay,#shop-the-look__wrapper .close-grid-products').on('click',function(){
        $('html').removeClass('lookbook-show');
        $('body').find('#shop-the-look__wrapper #'+lookDrawer).removeClass("show-looks");
      });
    }
    
  }
  homePage.announcementBarSlider();
  homePage.initJewelryInfo();
  homePage.customFullWidthSlider();
  homePage.initShopTheLookSlider();
  homePage.hoverSlickSlider();
  homePage.shopTheLook();

  var PrincesssPage = {
    initSliders: function() {
      var PrincessStylesSlider = $('.princess_slider');
      var ThreeBannerSlider = $('.princess-threeblock .display-flex');
      if(PrincessStylesSlider.length) {
        PrincessStylesSlider.slick({
          dots: false,
          arrows: true,
          prevArrow:false,
          nextArrow:'<button class="slick-next slick-arrow" aria-label="Next" type="button" >Flip Next Page<svg xmlns="http://www.w3.org/2000/svg" width="5.576" height="10.94" viewBox="0 0 5.576 10.94"><path id="Right_arrow" data-name="Right arrow" d="M.342,10.94A.342.342,0,0,1,.1,10.357L4.826,5.631a.229.229,0,0,0,0-.323L.1.584A.342.342,0,0,1,.584.1L5.309,4.826a.911.911,0,0,1,0,1.289L.584,10.84a.341.341,0,0,1-.242.1" fill="#12204a"/></svg></button>',
          infinite: true,
          speed: 1000,
          slidesToShow: 1,
          slidesToScroll: 1,
        ...
        $('.princess_slider_nav').slick({
          slidesToShow: 2,
          slidesToScroll: 1,
          asNavFor: PrincessStylesSlider,
          dots: false,
          arrows: false,
          rows:0,
          focusOnSelect: true
      ...
      if(ThreeBannerSlider.length && $(window).width() < 767) {
        ThreeBannerSlider.slick({
          dots: true,
          arrows: false,
          infinite: true,
          slidesToScroll: 1
  ...
  PrincesssPage.initSliders();
  
  // PDP
  var pdp = {
    initpdpChateaus_slider: function() {
      var swiper = new Swiper('.pdp-chateaus-slider', {
        slidesPerView: 3,
        spaceBetween: 18,
        centeredSlides: true,
        loop: true,
  ...
  var bodyChateauClass = $('body').hasClass('template_chateau');
  console.log(bodyChateauClass);
  if(bodyChateauClass){
    pdp.initpdpChateaus_slider();
  }
  ...
  $('.tabs .tabs-nav li').click(function() {
    ...
    $('[data-id="'+tab_id+'"]').siblings().removeClass('current');

    $(this).addClass('current');
    $('[data-id="'+tab_id+'"]').addClass('current');
    $('.product-grid-slider .grid--view-items').slick('setPosition');
  });

});


//Cart page priority shipping
$('#priorityShipping').on('change', function() {
  if($(this).is(':checked')) {
    Cookies.set('PriorityShipping', true);
  } else {
    Cookies.set('PriorityShipping', false);
  }
  
  var getPriorityShipping = Cookies.get('PriorityShipping');
  if(getPriorityShipping == 'true') {
  	$('#priorityShipping').prop('checked', true);
  }
});
