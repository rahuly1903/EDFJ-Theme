...
// 	alert("test");
  ...
  if (cart.item_count > 0) {
    for (var i = 0; i < cart.items.length; i++) {
      ...
      item = item.replace(/\{PRICE\}/g, theme.Currency.formatMoney((cart.items[i].price)*cart.items[i].quantity, theme.moneyFormat));
  ...
  var homePage = {
    ...
    initJewelryInfo: function() {
      $('.jewelry-info-section .btn-show-info').on('click', function (e){
        e.preventDefault();
        $(this).parent('.content').siblings('.jewelry-info').slideToggle();
...
//         });
    ...
    hoverSlickSlider: function() {
      var $this = $('.mm-product-grid-wrapper');
  ...
  var ajaxCartDrawer = {
    ...
    AjaxcartRender: function() {
      $.get('/cart?view=mm-ajax', function(cart) {
        ...
        var NewCartCount = parseInt($('#CartPopupHeading span').text().split(' ')[0]);
        if (NewCartCount > 0) {
  ...
  //   Ajax cart on PLP
  $(document).on('click', '.mm-product-info button[type="submit"]', function(e) {
    e.preventDefault();
