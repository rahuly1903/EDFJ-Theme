...
//   }
  ...
  $('.share-an-wishtlist .swym-custom-add-to-wishlist:not(.swym-added)').on('click',function(e){
    console.log("ready")
    ...
    var productData = {epi: parseInt(variantId), empi: parseInt(productId), type: productLevel};
    var $_this = $(this);
    window._swat.addToWishList(productData,function(r) {
      console.log('Added to wishlist');
      $_this.addClass("swym-added");
    });
  });

  $('.share-an-wishtlist .swym-custom-add-to-wishlist.swym-added').on('click',function(e){
    var productJson = $(this).data('product-json');
    var productId = $(this).data("product-id");
    var variantId = $('[name="id"]').find(":selected").val();
    var productLevel = window._swat.retailerSettings.General.ProductLevel;
    var productData = {epi: parseInt(variantId), empi: parseInt(productId), type: productLevel};
    var $_this = $(this);
    window._swat.removeFromWishList(productData,function(r) {
      console.log('removed from wishlist');
      $_this.removeClass("swym-added");
    });
  });
...
$(document).ready(function() {
  ...
  var ProductPage = {
    ...
    initAccordion: function() {
      $(".product_description--accordion .accordion-header:first-of-type").addClass("active").next().show();
  ...
  var promotionPopup = {
    promotionPopup: function() {
      ...
      $('#promotion-popup .popup-content form').submit( function(e){
        e.preventDefault();
        if($(this).find('input[type="email"]').val() !== ''){
          ...
          $.ajax({  
            ...
            error: function() {
              $('.error-wrapper').show().find('p').text('There was an error in submitting the form.');
            }    
          }); 
        }
        else {
...
//     console.log(1);
    if ($('#mhaRnProduct').children().length > 0 ) {
      console.log(1);
      myStopFunction();
      var htmlCode = '<img class="lazyload" src="https://cdn.shopify.com/s/files/1/0267/0523/1943/t/70/assets/Bag-blue.svg?v=16242431330005339556" data-sizes="auto"><span>Join waiting list</span>';
  ...
  function myTimer2() {
    ...
    if(count>=15){
      ...
      $.ajax({
        dataType: "json",
        url: "https://pro.ip-api.com/json/?fields&key=2FTD9IOnsjTmqWA",
        success: function(data){
...
//     alert(title.val());
    ...
    if(validation == true) {
      ...
      firstName.val(titelValue+' '+firstNameValue);
...
//     return false;
  ...
  $('#register___page form').on('submit', function(e) {
  ...
  $.ajax({
    ...
    data: {email:'rahul190303030304@gmail.com'},
    dataType: "text",
